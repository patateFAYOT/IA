...
void Learning_Bot::Update()
{
	...
	if (!isPossessed())
	{
		...
		if (m_pTargSys->isTargetPresent())
		{
			...
			//calling the CNN learnt model with current observations to know if the bot should shoot
			m_vecTargets = m_CNNModel.Update(m_vecObservations);

			bool canShoot = false;

			if (m_vecTargets.size() > 0)
			{
				canShoot = m_vecTargets[0];
			}
